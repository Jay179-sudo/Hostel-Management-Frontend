...
import { useRegisterSuperUserMutation } from '../../services/userAuthApi'
import { storeToken } from '../../services/LocalStorageService';

const SuperUserRegistration = () => {
  const [server_error, setServerError] = useState({})
  const navigate = useNavigate();
  const [registerSuperUser, { isLoading }] = useRegisterSuperUserMutation()
  const handleSubmit = async (e) => {
    ...
    const res = await registerSuperUser(actualData)
    if (res.error) {
